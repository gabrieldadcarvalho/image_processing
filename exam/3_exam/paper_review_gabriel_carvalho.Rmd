...
Para estimar a intensidade do pixel da imagem, utiliza-se a seguinte função:

$$
\hat{\mu}[n, m] = g^{-1}\left(\hat{\beta} + \sum_{i=0}^{p}\sum_{j=0}^{p} \hat{\phi}_{(i,j)}\, g\!\big(y[n-i,\,m-j]\big) + \sum_{k=0}^{q}\sum_{l=0}^{q} \hat{\theta}_{(k,l)}\, e[n-k,\,m-l]\right)
$$
